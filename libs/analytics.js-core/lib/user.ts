...
import { OWNER_PREFIX } from './constants';
...
User.defaults = {
  persist: true,
  cookie: {
    key: `${OWNER_PREFIX}_ajs_user_id`,
    oldKey: `${OWNER_PREFIX}_ajs_user`,
  },
  localStorage: {
    key: `${OWNER_PREFIX}_ajs_user_traits`,
  },
};
...
User.prototype.anonymousId = function (anonymousId?: string): string | User {
  ...
  // set / remove
  if (arguments.length) {
    store.set(`${OWNER_PREFIX}_ajs_anonymous_id`, anonymousId);
    this._setAnonymousIdInLocalStorage(anonymousId);
    return this;
  }

  // new
  anonymousId = store.get(`${OWNER_PREFIX}_ajs_anonymous_id`);
  if (anonymousId) {
  ...
  if (!this._options.localStorageFallbackDisabled) {
    ...
    anonymousId = localStorage.get(`${OWNER_PREFIX}_ajs_anonymous_id`);
    if (anonymousId) {
  ...
  store.set(`${OWNER_PREFIX}_ajs_anonymous_id`, anonymousId);
  this._setAnonymousIdInLocalStorage(anonymousId);
  return store.get(`${OWNER_PREFIX}_ajs_anonymous_id`);
};
...
User.prototype._setAnonymousIdInLocalStorage = function (id: string) {
  if (!this._options.localStorageFallbackDisabled) {
    localStorage.set(`${OWNER_PREFIX}_ajs_anonymous_id`, id);
  }
};
