...
var {
  TAG_URL,
  getViettelEndpoint,
} = require('../../analytics.js-core/lib/constants');
...
exports.sendJsonWithTimeoutVT = function (url, obj, headers, timeout, fn) {
  // only proceed with our new code path when cors is supported. this is
  // unlikely to happen in production, but we're being safe to preserve backward
  // compatibility.
  if (send.type !== 'xhr') {
...
Segment.prototype.initialize = function () {
  ...
  if (this.options.retryQueue) {
    // 'segmentio'
    this._lsqueue = new Queue('aicactus', queueOptions, function (item, done) {
      ...
      const vtURL = `${getViettelEndpoint()}/${splitURL[splitURL.length - 1]}`;
      // send with 10s timeout
      Segment.sendJsonWithTimeoutVT(
        vtURL,
        item.msg,
        item.headers,
        10 * 1000,
        function (err, res) {
          console.log('🚀 ~ file: index.js ~ line 215 ~ err', err);
          self.debug('sent %O, received %O', item.msg, [err, res]);
          // if (err) return done(err);
          done(null, res);
        },
...
Segment.prototype.normalize = function (msg) {
  ...
  if (this.options?.destinationSource) {
    msg.writeKey = `${this.options.apiKey}@${this.options.destinationSource}`;
  } else {
    msg.writeKey = `${this.options.apiKey}@web`;
  }

  if (this.options?.sdkType) {
    msg.sdkType = this.options.sdkType;
  }
